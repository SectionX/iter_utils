...
def test_flatten_length():
    results = []
    for k, v in flatten(nested_dict):
        results.append((k, v))

    assert len(results) == 6, "wrong flatten length for flatten(dict)"
...
def test_flatten_objects():
    ...
    check_keys = ['key1', 'key2', 'key3.key3_1', 'key3.key3_2', 'key4', 'key5.key5_1.key5_1_1']
